...
import com.lsx.onlineexamsystem.domain.*;
import com.lsx.onlineexamsystem.domain.Record;
import com.lsx.onlineexamsystem.service.*;
...
public class LoginController {
    ...
    public Object foreCheck(Student student, HttpServletRequest request){
        AjaxResult result=new AjaxResult();
        HttpSession session=request.getSession();
        Student stud=studentService.check(student);//StudentService.java中的check函数
        if(stud!=null){
    ...
    public String indexprexam(Model model){
        String notice_t = studentService.queryNotice();
        model.addAttribute("notice_t",notice_t);
        return "stage/prexamed";
    }

    //公告
    @RequestMapping("/templates/addNotice")
    public String addnotice(Student student){
        studentService.addNotice(student);
        return "redirect:/index";
    }

    //学生自己修改信息
    @RequestMapping("/templates/student/alterStudent")
